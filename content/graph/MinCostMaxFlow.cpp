 ...
 * Description: $min_cost_flow(s, t, f)$ computa o par $(fluxo, custo)$ com $max(fluxo) <= f$ que tenha $min(custo)$
 * $min_cost_flow(s, t)$: Fluxo maximo de custo minimo de s pra t
 * Se for um dag, da pra substituir o SPFA por uma DP pra naopagar $O(nm)$ no comeco
